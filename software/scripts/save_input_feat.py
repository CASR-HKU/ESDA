...
def main():
    ...
    os.makedirs(args.output_file + "_out", exist_ok=True)
    ...
    val_dataset = Dataset(cfg, mode="validation", cross_valid=4, shuffle=False)
    ...
    for idx in range(len(val_dataset)):
        ...
        #     mask[c[0], c[1]] = 1
        np.save(os.path.join(args.output_file, "{}.npy".format(idx)), data["histogram"].numpy())
        np.save(os.path.join(args.output_file + "_out", "{}.npy".format(idx)), np.array(data["label"]))
