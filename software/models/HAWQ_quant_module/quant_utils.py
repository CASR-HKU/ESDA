...
def linear_quantize_weight(input, scale, zero_point, inplace=False):
    scale = scale.view(1, -1)
    zero_point = zero_point.view(1, -1)
    # reshape scale and zeropoint for convolutional weights and activations
    if inplace:
...
class SymmetricQuantFunction(Function):
    ...
    @staticmethod
    def forward(ctx, x, k, specified_scale=None, isweight=False):
        """
        ...
        if isweight:
            new_quant_x = linear_quantize_weight(x, scale, zero_point, inplace=False)
        else:
    ...
    def backward(ctx, grad_output):
        ...
        scale = ctx.scale
        # print(grad_output.shape, scale.shape)
...
class fixedpoint_fn(Function):
    ...
    def forward(ctx, z, bitwidth, quant_mode, z_scaling_factor, case, pre_act_scaling_factor=None,
        ...
        with torch.no_grad():
            ...
            if case == 0:
                ...
                output = torch.round(output / (2.0 ** shift_bit))
    ...
    @staticmethod
    def backward(ctx, grad_output):
        return grad_output.clone() / ctx.z_scaling_factor, None, None, None, None, None, None, None, None, None, None
