...
#pragma HLS STREAM variable = token_1 depth = 2
    ...
    quantize_id_add<PO, OC, HEIGHT, WIDTH, PSUMW, AW, SCALEW, BIASW, SCALEIDW,
                    EXP>(pusm_1, act_id, act_out, scale_buffer, id_scale);
}
...
#pragma HLS STREAM variable = offset_0 depth = 9
    ...
    // conv
    conv_3x3_dw_kernel_serial<PI, IC, HEIGHT/2, WIDTH/2, AW, WW, PSUMW>(
        win_0, pusm_1, w_buffer);

    // quantize
    quantize<PI, IC, HEIGHT/2, WIDTH/2, PSUMW, AW, SCALEW, BIASW, EXP, 1>(
        pusm_1, act_out, scale_buffer);
}
...
void conv_1x1_3x3_dw_1x1_stride1(
    ...
    hls::stream<BundleT<PF_2, ap_int<AW>>> &act_out, 
...
#pragma HLS STREAM variable = token_2 depth = 2
    ...
    conv_1x1_module<PF_0, PF_1, IC, C, HEIGHT, WIDTH, AW, WW, PSUMW0, SCALEW0,
                    BIASW0, EXP, 1>(act_in, act_1, w_buffer_0,
                                 scale_buffer_0);
...
#pragma HLS STREAM variable = token_2 depth = 2
    ...
    // conv 1x1
    cout << "conv 1x1_2" << endl;
    conv_1x1_module<PF_1, PF_2, C, OC, HEIGHT/2, WIDTH/2, AW, WW, PSUMW2, SCALEW2,
                    BIASW2, EXP, 0>(act_2, act_out, w_buffer_2,
                                 scale_buffer_2);
}
...
#pragma HLS STREAM variable = token_2 depth = 2
    ...
    DO_PRAGMA(HLS STREAM variable = act_id depth = (WIDTH * 3) * OC / PF_2)
    ...
    cout << "conv 1x1" << endl;
    conv_1x1_module<PF_0, PF_1, IC, C, HEIGHT, WIDTH, AW, WW, PSUMW0, SCALEW0,
                    BIASW0, EXP, 1>(act_0, act_1, w_buffer_0,
                                 scale_buffer_0);
    ...
    conv_1x1_module_residual<PF_1, PF_2, C, OC, HEIGHT, WIDTH, AW, WW, PSUMW2,
        ...
        act_2, act_id, act_out, w_buffer_2, scale_buffer_2, id_scale);
}

// PI == IC
template <int PI, int PO, int OC, int HEIGHT, int WIDTH, int AW, int WW,
          int PSUMW, int SCALEW, int BIASW, int EXP>
void conv_3x3_first_layer(hls::stream<BundleT<PI, ap_int<AW>>> &act_in,
                          hls::stream<BundleT<PO, ap_int<AW>>> &act_out,
                          hls::stream<T_K> &token_in,
                          hls::stream<T_K> &token_out,
                          hls::stream<T_K> &token_stride2,
                          const ap_int<PI * WW> w_buffer[9][OC],
                          const ap_int<SCALEW + BIASW> scale_buffer[OC]) {
#pragma HLS DATAFLOW
...
#pragma HLS STREAM variable = token_1 depth = 2
    ...
    // linebuffer
    conv_3x3_line_buffer_first_layer<PI, PI, HEIGHT, WIDTH, AW>(
        act_in, win_0, token_in, token_0, token_stride2);

    conv_3x3_kernel_dsp_first_layer<PI, PO, OC, HEIGHT, WIDTH, AW, WW, PSUMW>(
