...
def append_prolouge_code(codes, common_para):
    ...
    codes["load"] += func_code_block(
        "read_sparse_input",
        [
            cfg_of(dflt_top_name, "PIC"),
            "CFG_AW",
            cfg_of(dflt_top_name, "IC"),
            cfg_of(dflt_top_name, "IH"),
            cfg_of(dflt_top_name, "IW"),
        ],
        ["act_in", afifo_of(dflt_top_name)],
    )
    # # load to token_fifo
    # codes["load"] += func_code_block(
    #     "M2S_mask",
    #     ["CFG_MW", cfg_of(dflt_top_name, "IH"), cfg_of(dflt_top_name, "IW")],
    #     ["mask", tfifo_of(dflt_top_name)],
    # )
    # para.h
    codes["para.h"] += [
        "#ifndef __PARA_H__\n",
...
def append_perlayer_code(codes, layer, prev_name, first_layer=False, last_layer=False):
    ...
    if layer["type"] == "conv":
        ...
        elif layer["kernel_size"] == 1:
            func_name = "conv_1x1_module"
            unique_tpl_list = def_k_list.copy()
            unique_arg_list = []
            warnings.warn("need to implement 1x1 conv")
        else:
    ...
    elif layer["type"] == "lin-pool":
        func_name = "global_avgpool_linear"
        def_k_list = []
        def_v_list = []
        unique_tpl_list = []
        unique_arg_list = []
        warnings.warn("need to implement Template and Arguments for last pool layer")
    else:
